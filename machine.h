...
public:
   ...
   //  オーバーロードを用いて、2種類の制御手法の確率
    /**
     * @fn ブロックを超えるまで走行させる。走らせる距離を引数にいれる必要がない。自己位置推定を利用しての制御
     */
    void move_p(double_t v);

    /**
     * @fn 指定した分の距離を前進。自己位置を参照しない制御
     */
    ...
    /**
     * @fn 自己位置推定の積算誤差が発生してしまうので、それを修正させる制御
     */
    void fit();
    ...
    /**
     * @fn 求心(目的座標に最短で向かう)手法のルールベースの基礎アルゴリズム
     */
